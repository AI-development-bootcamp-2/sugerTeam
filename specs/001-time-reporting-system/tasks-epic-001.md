...
- [ ] T007 Install all backend runtime dependencies (express, @prisma/client, jsonwebtoken, bcrypt, zod, cors, cookie-parser, multer, uuid) and dev dependencies (@types/express, @types/jsonwebtoken, @types/bcrypt, @types/cors, @types/cookie-parser, @types/multer, @types/uuid, prisma); create backend/src/app.ts exporting configured Express app with JSON body parser, cors (origin VITE_API_URL, credentials true), cookie-parser; create backend/src/server.ts starting on PORT with `GET /api/v1/health → 200 { status: "ok" }`: backend/package.json, backend/src/app.ts, backend/src/server.ts
